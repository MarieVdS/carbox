...
def save_abundances(
    ...
    # We use config.number_density as the divisor for fractional abundances
    n_h_nuclei = config.number_density
    ...
    for i, name in enumerate(species_names):
        # data[name] = solution.ys[:, i] / n_h_nuclei
...
def save_reaction_rates(
    ...
    # Note: If you are doing an outflow, remember to use dynamic n, T, and Av 
    # instead of the static config values here!
    data = {
        "time_seconds": times,
        "time_years": times / SPY,
        "number_density": config.number_density,
